...
func GetAll (w http.ResponseWriter, r *http.Request){
	...
	_ , err1 := db.Exec("insert into testdb.ads (name,link,price) values (?,?,?)","CAR","flick.com",1000000 )
	if err1 != nil{
		panic(err)
	}*/
	...
	db.QueryRow("SELECT created_at from testdb.ads where id=11").Scan(&myTime)
	...
	base_request+=" LIMIT 50 OFFSET " + strconv.Itoa(p.Offset)
	fmt.Println(base_request)
